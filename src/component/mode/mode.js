...
q('.mode-menu [data-mode]').forEach(btn =>
...
const setGlass = value => {
  ...
  q('[data-glass]').forEach(btn => btn.classList.toggle('active', btn.dataset.glass === value))
  q('[data-glass-icon]').forEach(el => (el.className = `fa-solid ${glassIcons[value]}`))
...
const syncGlass = () => {
  const dark = isDark()
  q('[data-glass], #glass-toggle, #glass-cycle, #glass-select-trigger').forEach(btn => (btn.disabled = !dark))
...
q('#glass-group [data-glass]').forEach(btn => btn.addEventListener('click', () => setGlass(btn.dataset.glass)))

g('glass-select-trigger')?.addEventListener('click', () =>
  document.querySelector('.glass-menu')?.classList.toggle('open'),
)

q('.glass-menu [data-glass]').forEach(btn =>
  btn.addEventListener('click', () => {
